...
def parameterWithMode(memory, i, mode):
    ...
    match this_mode:
        ...
        case 1:
            return parameter
        case _: # any unspecified should be considered as 0
            return memory[parameter]


def userInput(memory, i):
    """Awaits a int ans user input\n
    Stores the value at the cell given by memory[i + 1]"""
    while True:
        try:
...
def jumpIf(memory, i_after_condition, condition, mode):
    if condition:
        if len(mode) == 0 or mode.pop() == 0:
            ...
            return memory[i_after_condition]
    return i_after_condition + 1

def operation2(memory, i, mode, func):
    ...
    a = parameterWithMode(memory, i + 1, mode)
    b = parameterWithMode(memory, i + 2, mode)
        ...
        try:
            ...
            match next_opcode:
                case 1:
                    ...
                    operation2(memory, i, mode, lambda a, b: a + b)
                    i += 4
                case 2:
                ...
                case 3:
                    ...
                    i += 2
                case 4:
                    # prints the value from i + 1 after aplying the mode
                    value = parameterWithMode(memory, i + 1, mode)
                    print(value)
                    i += 2
                case 5:
                    arg = parameterWithMode(memory, i + 1, mode)
                    condition = bool(arg)
                    i = jumpIf(memory, i + 2, condition, mode)
                case 6:
                    arg = parameterWithMode(memory, i + 1, mode)
                    condition = not arg
                    i = jumpIf(memory, i + 2, condition, mode)
...
# asserts with automated input
# print(compute([3,9,8,9,10,9,4,9,99,-1,8]))
...
memory_result = compute(commands.copy())
print(f"The Execution was finished with the value {memory_result}.") if memory_result else print("The execution was unsuccessful")
