...
def param_modes(memory, position, index_nr, mode):
    if mode == 1:
        return memory[position + index_nr]
    elif mode == 0:
        return memory[memory[position + index_nr]]
    else:
        print("Error!")

def opcode_number(memory):
    position = 0
    while True:
        opcode_full = memory[position]

        opcode = opcode_full % 100
        mode_1 = (opcode_full // 100) % 10
        mode_2 = (opcode_full // 1000) % 10

        match opcode:
            case 1: #opcode from week 2
                param_1 = param_modes(memory, position, 1, mode_1)
                param_2 = param_modes(memory, position, 2, mode_2)
                memory[memory[position + 3]] = param_1 + param_2
                position += 4

            case 2: #opcode from week 2
                param_1 = param_modes(memory, position, 1, mode_1)
                param_2 = param_modes(memory, position, 2, mode_2)
                memory[memory[position + 3]] = param_1 * param_2
                position += 4
            ...
            case 3:
                ...
                memory[memory[position + 1]] = user_input
                position += 2

            case 4:
                case_4 = param_modes(memory, position, 1, mode_1)
                print(case_4)
                position += 2
