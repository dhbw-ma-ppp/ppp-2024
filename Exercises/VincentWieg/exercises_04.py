...
def listChange(listOfInt):
    ...
    while i < len(listOfInt):
        op_code = listOfInt[i] % 100
        mode_parameter_1 = (listOfInt[i] // 100) % 10
        mode_parameter_2 = (listOfInt[i] // 1000) % 10
        mode_parameter_3 = (listOfInt[i] // 10000) % 10
        
        if op_code == 1:  # add
            listOfInt[listOfInt[i + 3]] = get_value(i + 1, mode_parameter_1) + get_value(i + 2, mode_parameter_2)
            i += 4
        elif op_code == 2:  # multiply
            listOfInt[listOfInt[i + 3]] = get_value(i + 1, mode_parameter_1) * get_value(i + 2, mode_parameter_2)
            i += 4
        elif op_code == 3:  # input
            listOfInt[listOfInt[i + 1]] = int(input("Enter Input: "))
            i += 2
        elif op_code == 4:  # output
            print(get_value(i + 1, mode_parameter_1))
            i += 2
        elif op_code == 5:  # jump-if-true
            if get_value(i + 1, mode_parameter_1) != 0:
                i = get_value(i + 2, mode_parameter_2)
            else:
                i += 3
            ...
            if get_value(i + 1, mode_parameter_1) < get_value(i + 2, mode_parameter_2):
                listOfInt[listOfInt[i + 3]] = 1
            else:
                listOfInt[listOfInt[i + 3]] = 0
            i += 4
