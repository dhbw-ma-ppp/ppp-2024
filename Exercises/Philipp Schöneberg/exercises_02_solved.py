...
def list_manipulator (lst):
    for i, a in enumerate(lst[::4]):
        match lst[i*4]:
            case 1:
                lst[lst[i*4+3]] = lst[lst[i*4+1]]+lst[lst[i*4+2]]
            case 2:
                lst[lst[i*4+3]] = lst[lst[i*4+1]]*lst[lst[i*4+2]]
            case 99:
                break
            case _:
                print("The given opcode is unvalid.")
    return lst[0]

print(list_manipulator(commands))
...
lst_numbers, lst_mono_character = string_sorter("1", "2.0", "22")
print(lst_numbers)
print(lst_mono_character)

lst_numbers, lst_mono_character = string_sorter("1", "2.0", "22", ".", "x", "HalloWelt", "Hello World")
print(lst_numbers)
print(lst_mono_character)
