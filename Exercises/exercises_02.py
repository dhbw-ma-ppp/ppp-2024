...
def compute_oppcodes(test):
    oppcode = 0
    while True:
        if test[oppcode] == 1:
            test[test[oppcode+3]] = test[test[oppcode+1]] + test[test[oppcode+2]]
            oppcode += 4
        elif test[oppcode] == 2:
            test[test[oppcode+3]] = test[test[oppcode+1]] * test[test[oppcode+2]]
            oppcode += 4
        elif test[oppcode] == 99:
            break
    return test[0]

print("-------------------------------------------------------------------------")
print (f"the first number in the list after computation is: {compute_oppcodes(commands)}.")
print("-------------------------------------------------------------------------")
...
def sort_string(*args):
    ...
    for i in args:
        try:
            ...
            pass
        if len(i) == 1:
            output_chars.append(i)
    return [output_nums, output_chars]

def pretty_print_sort_string(result):
    conc = ", "
    print(f"contends of first list (numbers): {conc.join(result[0])}\ncontends of second list (single characters): {conc.join(result[1])}")
    print("-------------------------------------------------------------------------")
    
#Example 1: random stuff
pretty_print_sort_string(sort_string("100", "hello", "3.14", "A", "b", "0", "-42", "c", "!.7634", "0.5643435", "2", "9"))
#Example 2: special characters, empty strings and whitespaces
pretty_print_sort_string(sort_string("", "§", "/", "()$%&)", "!", "@", "#", "D", "  ", " 1", "2 ", " ", " a "))
#Example 3: floats and negative numbers
pretty_print_sort_string(sort_string("-1", "-10", "-5643", "-0563", "-0", "-9.764","3.14159", "2.718", "0.1"))