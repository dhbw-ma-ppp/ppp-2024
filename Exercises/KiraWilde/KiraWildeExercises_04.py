...
def simulate_computer (memory):
    pointer = 0
    while True:
        instruction = memory[pointer]
        #Extract the last two digits for the opcode
        opcode = instruction % 100
        #Extract the Extract the second digit for the mode
        mode1 = (instruction // 100) % 10
        #Extract the Extract the first digit for the mode
        ...
        match opcode:
            ...
            case 1:
                #Addition
                first_addend = get_value(memory, memory[pointer + 1], mode1)
                second_addend = get_value(memory, memory[pointer + 2], mode2)
                result_position = memory[pointer + 3]
                memory[result_position] = first_addend + second_addend
                pointer += 4
            case 2:
                #Multiplication
                first_factor = get_value(memory, memory[pointer + 1], mode1)
                second_factor = get_value(memory, memory[pointer + 2], mode2)
                result_position = memory[pointer + 3]
                memory[result_position] = first_factor * second_factor
                pointer += 4
            case 3:
                #User input
                result_position = memory[pointer + 1]
                user_input = int(input("Enter an intager: "))
                memory[result_position] = user_input
                pointer += 2
            case 4:
                #Output
                param1 = get_value(memory, memory[pointer + 1], mode1)
                print(param1)
                pointer += 2
            case 5:
                #Jump-if-true
                param1 = get_value(memory, memory[pointer + 1], mode1)
                param2 = get_value(memory, memory[pointer + 2], mode2)
                if (param1 != 0):
                    pointer = param2
                else :
                    pointer += 3
...
def get_value (memory, parameter, mode):
    if mode == 0:
        # Position mode (0): fetch value from address given by parameter
        return memory[parameter]
    else:
        # Immediate mode (1): use parameter directly as value
        return parameter
