...
def make_param_list(memory, pointer, parameter_num):        #calculate parameters in mode specified by the instructions
    output = []

    for i in range(1, parameter_num + 1):
        if (memory[pointer] // (10*10**i) % 10) == 0:        #parameter in position mode
            output.append(memory[memory[pointer+i]])    
...
def compute_oppcodes(memory):
    pointer = 0
    while True:
        if memory[pointer] % 100 == 1:
            parameter_num = 3
            parameters = make_param_list(memory, pointer, parameter_num)
            memory[memory[pointer+3]] = parameters[0] + parameters[1]
            pointer += (parameter_num + 1)
        elif memory[pointer] % 100 == 2:
            parameter_num = 3
            parameters = make_param_list(memory, pointer, parameter_num)
            memory[memory[pointer+3]] = parameters[0] * parameters[1]
            pointer += (parameter_num + 1)
        elif memory[pointer] % 100 == 3:    
            parameter_num = 1
            parameters = make_param_list(memory, pointer, parameter_num)
            memory[memory[pointer + 1]] = take_int_input()
            pointer += (parameter_num + 1)
        elif memory[pointer] % 100 == 4:
            parameter_num = 1
            parameters = make_param_list(memory, pointer, parameter_num)
            print(parameters[0])
            pointer += (parameter_num + 1)
        elif memory[pointer] % 100 == 5:
            parameter_num = 2
            parameters = make_param_list(memory, pointer, parameter_num)
            if parameters[0] != 0:
